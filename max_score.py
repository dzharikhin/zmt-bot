    ...
    with raw_report_file.open('rt') as raw_report:
        for line in (l.strip() for l in raw_report):
            ...
            for estimator in estimators.strip().split(";"):
                ...
                if (max_score := float(estimator_average) + float(estimator_deviation)) > 0.6 and float(estimator_average) >= 0.55:
                    filtered_estimators.append((max_score, estimator_average, estimator_deviation, data_config.strip(), estimator_config.strip()))

    count_by_data_config = defaultdict(int)
    count_by_estimator_config = defaultdict(int)
    for max_score, estimator_average, estimator_deviation, data_config, estimator_config in sorted(filtered_estimators, key=lambda r: r[0], reverse=True):
