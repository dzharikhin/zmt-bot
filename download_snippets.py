    ...
    *,
    ...
    while processed_in_try_tracks != 0 and (
        ...
        try:
            ...
            errors.append(e)
            print(
                f"{len(errors)} after {(datetime.datetime.now() - iteration_start).total_seconds()} seconds iteration, sleeping for {error_timeout_sec} sec"
            )
        ...
        for track in dataset.dataframe.itertuples():
            ...
            try:
                ...
                if preview_link:
                    ...
                    dataset.add_snippet_path(track.spotify_id, "FAILED")
                processed_tracks += 1
