...
if __name__ == "__main__":
    results = []
    for frames in [-48, 0, 12, 24, 48, 96]:
        ...
        for contamination in [0, 0.05, 0.1, 0.2, 0.25]:
            ...
            for model_name, model in ([(f"svm({frames=},{nu=})", OneClassSVM(nu=nu)) for nu in [0.1, 0.2, 0.3, 0.5]]
                                      + [(f"forest({frames=},{contamination_safe=},{estimators=})", IsolationForest(contamination=contamination_safe, n_estimators=estimators)) for estimators in [100, 50, 10, initial_data.shape[1] // 2, initial_data.shape[1] // 10]]
                                      + [(f"lof({frames=},{contamination_safe=},{leaf_size=},{n_neighbors=})", LocalOutlierFactor(novelty=True, contamination=contamination_safe, leaf_size=leaf_size, n_neighbors=n_neighbors)) for leaf_size, n_neighbors in product([30, initial_data.shape[0] // 10, initial_data.shape[0] // 2], [20, 10, math.ceil(initial_data.shape[0] * contamination) if contamination > 0 else 5, initial_data.shape[0] // 20])]
            ):
                ...
                for tries in range(10):
                    ...
                    model, accuracy = _train_dissimilar_model(
                        train_data=train_data, test_data=test_data, model=model, contamination_fraction=contamination, scale=True
                    )
                    try_accuracies.append(accuracy)
                results.append((float(np.mean(try_accuracies)), float(np.std(try_accuracies)), model_name, frames, contamination))

    print()
    for mean_accuracy, accuracy_variance, model_name, frames, contamination in sorted(results, reverse=True, key=lambda r: r[0]):
        print(f"Stats for {model_name=} on data [{frames=},{contamination=}]: accuracy={mean_accuracy:.3f}(+-{accuracy_variance:.3f})")
