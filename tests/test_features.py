...
class TestEssentiaExtraction:
    ...
    def test_extract_essentia_features_schema_width(self, tmp_path):
        y = np.random.default_rng(1).standard_normal(44100 * 3).astype(np.float32)
        audio_path = tmp_path / "test.wav"
        sf.write(audio_path, y, 44100)
        ...
        expected = sum(length for _, length, _ in audio_features._DESCRIPTOR_SCHEMA)
...
class TestAssertSchemaDimConsistent:
    ...
    def test_real_schema_with_profile_passes(self):
        import pathlib
...
class TestKeyScaleSchemaEntries:
    ...
    def test_entries_are_schema_tail_after_beats_position(self):
        names = [name for name, _, _ in audio_features._DESCRIPTOR_SCHEMA]
        idx = names.index("rhythm.beats_position")
        assert names[idx + 1 :] == [name for name, _, _ in KEY_SCALE_SCHEMA_ENTRIES]

    def test_schema_shape_after_frame_removal(self):
        schema = audio_features._DESCRIPTOR_SCHEMA
        assert len(schema) == 560
        assert sum(length for _, length, _ in schema) == 4380
        assert all(not name.startswith("frames.") for name, _, _ in schema)
    ...
    def test_family_layout_covers_new_tonal_dims(self):
        ...
        assert schema_total == 4380
        assert layout[-2] == ("rhythm", 4364, 4368)
        assert layout[-1] == ("tonal", 4368, schema_total)
