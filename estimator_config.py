...
if __name__ == "__main__":
    ...
    PROPS = ("suod_combination", "feature_bagging_type", "feature_bagging_n_estimators", "lscp_n_bins", "lscp_local_region_size", "loda_n_random_cuts")
    ROWS_TO_CHECK =(
    ...
    with raw_report_file.open('rt') as raw_report:
        ...
        for line_no, line in enumerate((l.strip() for l in raw_report), 1):
            ...
            for estimator in estimators.strip().split(";"):
                ...
                estimator_params = dict([tuple(param.split("=", 2)) for param in estimator_param_line.split("]", 2)[0].split(",")])
                for target in PROPS:
                    ...
                    tree[estimator_type][tuple(target_reduced_params.items())][target][target_param_value] = (estimator_average, estimator_deviation)

            meaningful_variations = {((estimator_type,), *reduced_params): param_target
            ...
            # print()
            print(f"{data_config.strip()=}:")
            for prop, all_variations_for_prop in groupby(sorted(meaningful_variations.values(), key=lambda d: [*d.keys()][0]), lambda d: [*d.keys()][0]):
                ...
                for variation in all_variations_for_prop:
                    best_by_average = list(sorted(variation[prop].items(), key=lambda i: float(i[1][0]), reverse=True))[0][0]
                    best_prop_by_average_counter[best_by_average] += 1
                    best_by_max = list(sorted(variation[prop].items(), key=lambda i: float(i[1][0]) + float(i[1][1]), reverse=True))[0][0]
                    best_prop_by_max_counter[best_by_max] += 1
                by_average_stat = ",".join([f"{value}->{best_count}/{len(all_variations_for_prop)}({best_count / len(all_variations_for_prop):.3f})" for value, best_count in sorted(best_prop_by_average_counter.items(), key=lambda i: i[1], reverse=True)])
                by_max_stat = ",".join([f"{value}->{best_count}/{len(all_variations_for_prop)}({best_count / len(all_variations_for_prop):.3f})" for value, best_count in sorted(best_prop_by_max_counter.items(), key=lambda i: i[1], reverse=True)])
