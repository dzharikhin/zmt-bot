...
def filter_outliers_per_genre(
    *,
    ...
    outliners = set()
    outliners_rows = [("genre", "out/total", "ids")]
    for genre, dataset in (
        ...
        X = dataset.select(pl.all().exclude(TRACK_ID_COLUMN_NAME, GENRE_COLUMN_NAME))
    ...
    print(f"Saved filtered genre dataset to {result_path}")
    outliners_store_path = result_path.parent.joinpath(
        f"{result_path.stem}-outliners.csv"
    )
    with outliners_store_path.open(mode="wt") as outliners_file:
        outliners_writer = csv.writer(outliners_file)
        outliners_writer.writerows(outliners_rows)
    print(f"Saved excluded outliners per genre to {outliners_store_path}")
