...
import requests
from lxml.etree import LxmlSyntaxError, XMLSyntaxError
        ...
        for track in dataset.dataframe.itertuples():
            if not force:
                ...
                if snippet_path.exists():
                    dataset.add_snippet_path(track.spotify_id, snippet_path)
                if dataset.has_snippet(track):
                    print(
            ...
            try:
                ...
                processed_tracks += 1
            except (XMLSyntaxError, LxmlSyntaxError, KeyError, ValueError) as e:
                print(
...
def _download_snippet(
    ...
    snippet = _get_snippet_path(store_path, track)
...
def _get_snippet_path(store_path: pathlib.Path, track: Track) -> pathlib.Path:
    return store_path.joinpath(f"{track.spotify_id}.mp3")
